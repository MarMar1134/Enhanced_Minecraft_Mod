...
public class SoulAlloyCategory implements IRecipeCategory<AlloyingFurnaceRecipe> {
    ...
    public static final RecipeType<AlloyingFurnaceRecipe> SOUL_ALLOY_FURNACE_RECIPE_RECIPE_TYPE =
            new RecipeType<>(UID, AlloyingFurnaceRecipe.class);

    private final IDrawable background;
    private final IDrawable icon;

    public SoulAlloyCategory(IGuiHelper helper) {
        this.background = helper.createDrawable(TEXTURE, 0, 0, 176, 82);
        this.icon = helper.createDrawableIngredient(VanillaTypes.ITEM_STACK, new ItemStack(ModBlocks.SOUL_ALLOY_FURNACE.get()));
    }

    @Override
    public RecipeType<AlloyingFurnaceRecipe> getRecipeType() {
        return SOUL_ALLOY_FURNACE_RECIPE_RECIPE_TYPE;
    }
